...
export function PolicyForm({ policy, propertyOptions }: { policy?: Record<string, unknown> | null; propertyOptions: { id: string; name: string }[] }) {
  return (
    <>
      ...
      {policy ? <form action={deletePoliciesAction} className="mt-4"><input type="hidden" name="id" value={String(policy.id)} /><button type="submit" className="rounded-xl border border-[#e2b0b0] bg-[#fbeeee] px-4 py-2 text-sm font-medium text-[#8f3f3f]">Delete Policy</button></form> : null}
    </>
  );
}
