...
export function AttractionForm({ attraction, propertyOptions }: { attraction?: Record<string, unknown> | null; propertyOptions: { id: string; name: string }[] }) {
  return (
    <>
      ...
      {attraction ? <form action={deleteAttractionsAction} className="mt-4"><input type="hidden" name="id" value={String(attraction.id)} /><button type="submit" className="rounded-xl border border-[#e2b0b0] bg-[#fbeeee] px-4 py-2 text-sm font-medium text-[#8f3f3f]">Delete Attraction</button></form> : null}
    </>
  );
}
