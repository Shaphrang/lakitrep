...
type Col<T> = { key: keyof T; label: string; sortable?: boolean; format?: "currency" | "percent_1" | "nights_1" };

function formatValue(value: unknown, format?: Col<never>["format"]) {
  const numeric = Number(value ?? 0);
  if (format === "currency") return `₹${numeric.toLocaleString("en-IN")}`;
  if (format === "percent_1") return `${numeric.toFixed(1)}%`;
  if (format === "nights_1") return `${numeric.toFixed(1)} nights`;
  return String(value ?? "—");
}
...
export function ReportTableClient<T extends { id: string }>({
  ...
  const filtered = useMemo(() => {
    ...
    let data = rows;
    if (q) data = data.filter((row) => searchKeys.some((k) => String(row[k] ?? "").toLowerCase().includes(q)));
    return [...data].sort((a, b) => {
      const order = String(a[sortBy]).localeCompare(String(b[sortBy]), undefined, { numeric: true });
    ...
    <section className="space-y-3 rounded-2xl border border-[#ddd4c6] bg-white p-4">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full text-left text-sm">
          ...
          <tbody>{paged.map((row) => <tr key={row.id} className="border-t border-[#eee6da]">{columns.map((c) => <td key={String(c.key)} className="whitespace-nowrap px-3 py-2">{formatValue(row[c.key], c.format)}</td>)}</tr>)}</tbody>
        </table>
      </div>
