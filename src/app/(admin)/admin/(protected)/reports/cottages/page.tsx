...
export default async function CottagesReportPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  <ReportTableClient title="Cottage Performance" fileName="cottages-report.csv" rows={rows} searchKeys={["cottageName"]} columns={[{key:"cottageName",label:"Cottage"},{key:"totalBookings",label:"Bookings",sortable:true},{key:"totalNightsBooked",label:"Nights",sortable:true},{key:"occupancyRate",label:"Occupancy",sortable:true,render:(r)=>`${r.occupancyRate.toFixed(1)}%`},{key:"totalRevenue",label:"Revenue",sortable:true,render:(r)=>`₹${r.totalRevenue.toLocaleString('en-IN')}`},{key:"totalCollection",label:"Collection",render:(r)=>`₹${r.totalCollection.toLocaleString('en-IN')}`},{key:"outstandingAmount",label:"Outstanding",render:(r)=>`₹${r.outstandingAmount.toLocaleString('en-IN')}`},{key:"averageRevenuePerBooking",label:"Avg/Booking",render:(r)=>`₹${r.averageRevenuePerBooking.toLocaleString('en-IN')}`},{key:"averageStayDuration",label:"Avg Stay",render:(r)=>`${r.averageStayDuration.toFixed(1)} nights`},{key:"cancellationCount",label:"Cancelled"}]} />
  </div>;
}
