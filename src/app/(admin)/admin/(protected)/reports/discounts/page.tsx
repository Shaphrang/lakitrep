...
export default async function DiscountsReportPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="space-y-4"><AdminPageHeader title="Discounts Report" description="Discount impact and revenue leakage tracking." /><ReportsNav />
    ...
    <ReportTableClient title="Discounted Bookings" fileName="discounts-report.csv" rows={rows} searchKeys={["bookingCode","guestName","cottageName"]} columns={[{key:"bookingCode",label:"Booking"},{key:"guestName",label:"Guest"},{key:"cottageName",label:"Cottage"},{key:"originalAmount",label:"Original",render:(r)=>`₹${r.originalAmount.toLocaleString('en-IN')}`},{key:"discountAmount",label:"Discount",render:(r)=>`₹${r.discountAmount.toLocaleString('en-IN')}`},{key:"discountReason",label:"Reason"},{key:"finalAmount",label:"Final",render:(r)=>`₹${r.finalAmount.toLocaleString('en-IN')}`},{key:"approvedBy",label:"Approved By"},{key:"date",label:"Date"}]} />
  </div>;
}
