...
export default async function ForecastingReportPage() {
  ...
  return <div className="space-y-4"><AdminPageHeader title="Forecasting Report" description="Forward-looking expected business from upcoming non-cancelled bookings." /><ReportsNav />
    ...
    <ReportTableClient title="Forecast windows" fileName="forecasting-report.csv" rows={views} searchKeys={["window"]} columns={[{key:"window",label:"Window"},{key:"upcomingBookings",label:"Upcoming Bookings"},{key:"expectedRevenue",label:"Expected Revenue",render:(r)=>`₹${r.expectedRevenue.toLocaleString('en-IN')}`},{key:"expectedCollection",label:"Expected Collection",render:(r)=>`₹${r.expectedCollection.toLocaleString('en-IN')}`},{key:"expectedOccupancy",label:"Expected Occupancy",render:(r)=>`${r.expectedOccupancy.toFixed(1)}%`},{key:"upcomingCheckins",label:"Check-ins"},{key:"upcomingCheckouts",label:"Check-outs"},{key:"availableCottageNights",label:"Available Cottage Nights"}]} />
  </div>;
}
