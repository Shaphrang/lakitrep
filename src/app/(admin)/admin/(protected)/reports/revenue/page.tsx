...
export default async function RevenueReportPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  <ReportTableClient title="Revenue Details" fileName="revenue-report.csv" rows={bookings} searchKeys={["bookingCode","guestName","cottageName","invoiceNumber"]} columns={[
    { key: "bookingCode", label: "Booking" },{ key: "guestName", label: "Guest" },{ key: "cottageName", label: "Cottage" },{ key: "checkIn", label: "Check-in" },{ key: "checkOut", label: "Check-out" },{ key: "baseAmount", label: "Base", sortable: true, format: "currency_inr" },{ key: "extraCharges", label: "Extra", sortable:true, format: "currency_inr" },{ key: "discount", label: "Discount", sortable:true, format: "currency_inr" },{ key: "tax", label: "Tax", sortable:true, format: "currency_inr" },{ key: "finalAmount", label: "Final Bill", sortable:true, format: "currency_inr" },{ key: "paidAmount", label: "Paid", sortable:true, format: "currency_inr" },{ key: "outstandingAmount", label: "Balance", sortable:true, format: "currency_inr" },{ key: "paymentStatus", label: "Payment" },{ key: "invoiceStatus", label: "Invoice" },
  ]} />
  </div>;
}
