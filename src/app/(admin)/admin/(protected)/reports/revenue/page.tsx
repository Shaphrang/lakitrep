...
export default async function RevenueReportPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  <ReportTableClient title="Revenue Details" fileName="revenue-report.csv" rows={bookings} searchKeys={["bookingCode","guestName","cottageName","invoiceNumber"]} columns={[
    { key: "bookingCode", label: "Booking" },{ key: "guestName", label: "Guest" },{ key: "cottageName", label: "Cottage" },{ key: "checkIn", label: "Check-in" },{ key: "checkOut", label: "Check-out" },{ key: "baseAmount", label: "Base", sortable: true, render:(r)=>`₹${r.baseAmount.toLocaleString('en-IN')}` },{ key: "extraCharges", label: "Extra", sortable:true, render:(r)=>`₹${r.extraCharges.toLocaleString('en-IN')}` },{ key: "discount", label: "Discount", sortable:true, render:(r)=>`₹${r.discount.toLocaleString('en-IN')}` },{ key: "tax", label: "Tax", sortable:true, render:(r)=>`₹${r.tax.toLocaleString('en-IN')}` },{ key: "finalAmount", label: "Final Bill", sortable:true, render:(r)=>`₹${r.finalAmount.toLocaleString('en-IN')}` },{ key: "paidAmount", label: "Paid", sortable:true, render:(r)=>`₹${r.paidAmount.toLocaleString('en-IN')}` },{ key: "outstandingAmount", label: "Balance", sortable:true, render:(r)=>`₹${r.outstandingAmount.toLocaleString('en-IN')}` },{ key: "paymentStatus", label: "Payment" },{ key: "invoiceStatus", label: "Invoice" },
  ]} />
  </div>;
}
