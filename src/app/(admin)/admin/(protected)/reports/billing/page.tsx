...
export default async function BillingInvoicesReportPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  <ReportTableClient title="Invoices" fileName="billing-invoices-report.csv" rows={bookings} searchKeys={["invoiceNumber","bookingCode","guestName"]} columns={[
    { key: "invoiceNumber", label: "Invoice #", sortable: true },{ key: "bookingCode", label: "Booking" },{ key: "guestName", label: "Guest" },{ key: "invoiceDate", label: "Invoice Date" },{ key: "baseAmount", label: "Base", render:(r)=>`₹${r.baseAmount.toLocaleString('en-IN')}` },{ key: "extraCharges", label: "Extra", render:(r)=>`₹${r.extraCharges.toLocaleString('en-IN')}` },{ key: "discount", label: "Discount", render:(r)=>`₹${r.discount.toLocaleString('en-IN')}` },{ key: "tax", label: "Tax", render:(r)=>`₹${r.tax.toLocaleString('en-IN')}` },{ key: "finalAmount", label: "Final", render:(r)=>`₹${r.finalAmount.toLocaleString('en-IN')}` },{ key: "paidAmount", label: "Paid", render:(r)=>`₹${r.paidAmount.toLocaleString('en-IN')}` },{ key: "outstandingAmount", label: "Balance", render:(r)=>`₹${r.outstandingAmount.toLocaleString('en-IN')}` },{ key: "invoiceStatus", label: "Invoice Status" },{ key: "paymentStatus", label: "Payment Status" },
  ]} />
  </div>;
}
