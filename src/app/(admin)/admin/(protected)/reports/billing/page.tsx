...
export default async function BillingInvoicesReportPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  <ReportTableClient title="Invoices" fileName="billing-invoices-report.csv" rows={bookings} searchKeys={["invoiceNumber","bookingCode","guestName"]} columns={[
    { key: "invoiceNumber", label: "Invoice #", sortable: true },{ key: "bookingCode", label: "Booking" },{ key: "guestName", label: "Guest" },{ key: "invoiceDate", label: "Invoice Date" },{ key: "baseAmount", label: "Base", format:"currency" },{ key: "extraCharges", label: "Extra", format:"currency" },{ key: "discount", label: "Discount", format:"currency" },{ key: "tax", label: "Tax", format:"currency" },{ key: "finalAmount", label: "Final", format:"currency" },{ key: "paidAmount", label: "Paid", format:"currency" },{ key: "outstandingAmount", label: "Balance", format:"currency" },{ key: "invoiceStatus", label: "Invoice Status" },{ key: "paymentStatus", label: "Payment Status" },
  ]} />
  </div>;
}
