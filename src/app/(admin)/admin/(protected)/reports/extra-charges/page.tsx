...
export default async function ExtraChargesReportPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="space-y-4"><AdminPageHeader title="Extra Charges Report" description="Charge analytics for ancillary revenue." /><ReportsNav />
    ...
    <ReportTableClient title="Extra Charge Entries" fileName="extra-charges-report.csv" rows={ds.charges} searchKeys={["bookingCode","guestName","cottage","chargeType","description"]} columns={[{key:"bookingCode",label:"Booking"},{key:"guestName",label:"Guest"},{key:"cottage",label:"Cottage"},{key:"chargeType",label:"Charge Type"},{key:"description",label:"Description"},{key:"amount",label:"Amount",format:"currency"},{key:"dateAdded",label:"Date Added"}]} />
  </div>;
}
