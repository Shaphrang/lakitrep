...
export default async function PaymentsCollectionsReportPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  <ReportTableClient title="Payment Transactions" fileName="payments-collections-report.csv" rows={payments} searchKeys={["bookingCode","guestName","phone","method","transactionReference"]} columns={[
    { key: "paymentDate", label: "Payment Date", sortable: true },{ key: "bookingCode", label: "Booking", sortable: true },{ key: "guestName", label: "Guest" },{ key: "phone", label: "Phone" },{ key: "method", label: "Method", sortable: true },{ key: "amount", label: "Amount", sortable: true, render:(r)=>`₹${r.amount.toLocaleString('en-IN')}` },{ key: "collectedBy", label: "Collected By" },{ key: "transactionReference", label: "Txn Ref" },{ key: "status", label: "Status" },{ key: "remarks", label: "Remarks" },
  ]} />
  </div>;
}
