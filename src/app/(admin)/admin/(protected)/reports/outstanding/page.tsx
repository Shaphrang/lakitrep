...
export default async function OutstandingDuesReportPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  <ReportTableClient title="Outstanding Bookings" fileName="outstanding-dues-report.csv" rows={dues.map((d)=>({...d,dueSince:d.checkOut<today?d.checkOut:d.checkIn,whatsappReminder:d.phone!=='-'?`https://wa.me/${d.phone.replace(/\D/g,'')}`:'—'}))} searchKeys={["bookingCode","guestName","phone","cottageName","source"]} columns={[
    { key: "bookingCode", label: "Booking" },{ key: "guestName", label: "Guest" },{ key: "phone", label: "Phone" },{ key: "cottageName", label: "Cottage" },{ key: "checkIn", label: "Check-in" },{ key: "checkOut", label: "Check-out" },{ key: "finalAmount", label: "Total Bill", render:(r)=>`₹${r.finalAmount.toLocaleString('en-IN')}` },{ key: "paidAmount", label: "Paid", render:(r)=>`₹${r.paidAmount.toLocaleString('en-IN')}` },{ key: "outstandingAmount", label: "Due", render:(r)=>`₹${r.outstandingAmount.toLocaleString('en-IN')}` },{ key: "dueSince", label: "Due Since" },{ key: "paymentStatus", label: "Payment" },{ key: "source", label: "Source" },{ key: "whatsappReminder", label: "WhatsApp" },
  ]} />
  </div>;
}
